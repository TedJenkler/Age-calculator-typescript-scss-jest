...
function Info({ now, selectedDate }: InfoProps) {
  ...
  return (
    <div className="info">
      <p aria-label="years"><span>{diff.year ? diff.year : "--"} </span>years</p>
      <p aria-label="months"><span>{diff.month ? diff.month : "--"} </span>months</p>
      <p aria-label="days"><span>{diff.day ? diff.day : "--"} </span>days</p>
    </div>
  );
}
